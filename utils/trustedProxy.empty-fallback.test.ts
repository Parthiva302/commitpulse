...
describe('trustedProxy - Edge Cases & Empty/Missing Inputs Verification', () => {
  ...
  it('isIPv4 returns false for empty, malformed, or non-IP strings', () => {
  ...
  it('ip4ToInt correctly converts an IP to its integer representation', () => {
    expect(ip4ToInt('0.0.0.0')).toBe(0);
    expect(ip4ToInt('127.0.0.1')).toBe(2130706433);
    expect(ip4ToInt('255.255.255.255')).toBe(4294967295);
  });

  it('isIPv4InCidr gracefully rejects invalid CIDR notation without throwing', () => {
  ...
  it('handles empty configs, wildcard entries, and exact matches correctly', () => {
    expect(
  ...
  it('loadTrustedProxyConfig returns sensible defaults when no env vars are set in production', () => {
